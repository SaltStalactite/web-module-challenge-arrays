...
function getFlavorByIndex(array, flavorIndex) {
  return array[flavorIndex];
}
...
function getAverageWordLength(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    average += words.length / array.length;
  }
  return average;
}
...
function getRandomFlavors(array1, array2, array3, array4) {
  let possibleFlavors = array1.concat(array2, array3, array4);
  let randomFlavors = [];
  for (let i = 0; i < 32; i++) {
    randomFlavors.push(possibleFlavors[Math.floor(Math.random() * possibleFlavors.length)])
  }
